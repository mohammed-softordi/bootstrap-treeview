!function (a, b, c, d) {
    ...
    f.settings = {
        ...
        indentNode: !1,
        customSelectedStyle: d,
        customExpandedStyle: d,
        expandedColor: d,
        expandedBackColor: d,
        decorator: d,
        hoverTextColor: d,
    ...
    var g = function (b, c) {
        return this.$element = a(b), this.elementId = b.id, this.styleId = this.elementId + "-style", this.init(c), {
            ...
            setData: a.proxy(this.setData, this)
        }
    };
    g.prototype.init = function (b) {
        ...
        this.initialized || (this.$element.addClass(e), this.$wrapper = a(this.template.list), this.injectStyle(), this.initialized = !0), this.$element.empty().append(this.$wrapper.empty()), this.buildTree(this.tree, 0), this.options.customTemplate && this.$wrapper.append(this.options.customTemplate), this.$element.trigger("treeRendered", a.extend(!0, {}, this.tree))
    }, g.prototype.buildTree = function (b, c) {
        if (b) {
            c += 1;
            var d = this;
            a.each(b, function (b, e) {
                if (d.options.decorator && d.options.decorator instanceof Function && (e = d.options.decorator(e, c)), !e) return !0;
                var f = a(d.template.item).addClass("node-" + d.elementId).addClass(e.state.checked ? "node-checked" : "").addClass(e.state.disabled ? "node-disabled" : "").addClass(e.state.selected ? "node-selected" : "").addClass(e.state.expanded ? "node-expanded" : "").addClass(e.searchResult ? "search-result" : "").attr("data-nodeid", e.nodeId).attr("style", d.buildStyleOverride(e));
                e.selector && f.addClass(e.selector);
                for (var g = 0; g < c - 1; g++) d.options.indentNode ? f.css(d.buildNodeIndentStyle(f, c)) : f.append(d.template.indent);
                ...
                if (d.options.showCheckbox) {
                    var h = ["check-icon"];
                    e.state.checked ? h.push(d.options.checkedIcon) : h.push(d.options.uncheckedIcon), f.append(a(d.template.icon).addClass(h.join(" ")))
                }
                d.options.enableLinks && e.selectable ? f.append(a(d.template.link).attr("href", e.href).append(e.text)) : f.append(e.text), d.options.showTags && e.tags && a.each(e.tags, function (b, c) {
                    f.append(a(d.template.badge).append(c))
                });
                var h = [];
                return e.nodes ? (h.push("expand-icon"), e.state.expanded ? h.push(d.options.collapseIcon) : h.push(d.options.expandIcon)) : h.push(d.options.emptyIcon), f.append(a(d.template.icon).addClass(h.join(" "))), d.$wrapper.append(f), e.nodes && e.state.expanded && !e.state.disabled ? d.buildTree(e.nodes, c) : void 0
            })
        }
    }, g.prototype.buildStyleOverride = function (a) {
        if (a.state.disabled) return "";
        var b = a.color, c = a.backColor;
        if (this.options.highlightSelected && a.state.selected) {
            if (this.options.customSelectedStyle) return this.options.customSelectedStyle;
            this.options.selectedColor && (b = this.options.selectedColor), this.options.selectedBackColor && (c = this.options.selectedBackColor)
        }
        if (this.options.highlightSearchResults && a.searchResult && !a.state.disabled && (this.options.searchResultColor && (b = this.options.searchResultColor), this.options.searchResultBackColor && (c = this.options.searchResultBackColor)), this.options.highlightSelected && a.state.expanded) {
            if (this.options.customExpandedStyle) return this.options.customExpandedStyle;
            this.options.expandedColor && (b = this.options.expandedColor), this.options.expandedBackColor && (c = this.options.expandedBackColor)
        }
        return "color:" + b + ";background-color:" + c + ";"
    }, g.prototype.buildNodeIndentStyle = function (a, b) {
        var c = 3 === b ? 0 : 5 * (b - 2);
        return {"margin-left": b > 2 ? 10 * b + c + "px" : "10px", "margin-right": 0}
        ...
        return this.options.color && (a += "color:" + this.options.color + ";"), this.options.backColor && (a += "background-color:" + this.options.backColor + ";"), this.options.showBorder ? this.options.borderColor ? a += "border:1px solid " + this.options.borderColor + ";" : this.options.customSelectedStyle && (a += this.options.customSelectedStyle + ";") : a += "border:none;", a += "}", this.options.onhoverColor && (a += ".node-" + this.elementId + ":not(.node-disabled):hover{background-color:" + this.options.onhoverColor + ";}"), this.options.hoverTextColor && (a += ".node-" + this.elementId + ":not(.node-disabled):hover{color:" + this.options.hoverTextColor + ";}"), this.css + a
        ...
        var b = this.identifyNode(a), c = this.getParent(b);
        return (c ? c.nodes : this.tree).filter(function (a) {
        ...
        if ((b = a.extend({}, f.options, b)) && b.levels) this.expandLevels(this.tree, b.levels, b); else {
        ...
        d = a.extend({}, f.options, d), a.each(b, a.proxy(function (a, b) {
            this.setExpandedState(b, c > 0, d), b.nodes && this.expandLevels(b.nodes, c - 1, d)
        }, this))
        ...
        return a.grep(this.nodes, function (a) {
            ...
            if ("string" == typeof f) return f.match(new RegExp(b, c))
        })
    ...
    }, g.prototype.setData = function (b) {
        this.tree = a.extend(!0, [], options.data), this.setInitialStates({nodes: this.tree}, 0), this.render()
    };
    var h = function (a) {
